...
```bash
node scripts/new-page/neighbour-scores.mjs --from a.html,b.html,c.html --save /tmp/before.json
node scripts/new-page/inject-backlinks.mjs scripts/page-configs/<slug>.json --from a.html,b.html,c.html
node scripts/new-page/neighbour-scores.mjs --from a.html,b.html,c.html --compare /tmp/before.json
...
`links[] updated` when that happened. `neighbour-scores.mjs`
audits each touched page against its own keyword (from its page-config if it has one,
else its H1; pass `file.html=keyword` to override) and `--compare` flags any drop — a
backlink should never lower a neighbour's score, and if it did, the card was appended in
the wrong place.
